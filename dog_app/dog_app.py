...
import zipfile
with zipfile.ZipFile('dogImages.zip','r') as myzip:
    myzip.extractall()
with zipfile.ZipFile('lfw.zip','r') as myzip:
    myzip.extractall()
...
# 回答：human_files的前100张图像中，能够检测到的人脸图像占比为100%  
# 
# 
# - `dog_files` 的前100张图像中，能够检测到**人脸**的图像占比多少？  
# 回答：dog_files 的前100张图像中，能够检测到人脸图像的占比是12%
# 
...
for face in dog_files_short:
    if face_detector(face):
        count_dog+=1
print('the rate of detect the face from human_files is {:.0%} '.format(count_human/len(human_files_short)))
print('the rate of detect the face from dog_files is {:.0%} '.format(count_dog/len(dog_files_short)))
...
# __回答:__  
# 
# 在实际使用中，用户不一定能够提供含有清晰面部特征的人脸图像，所以这样的要求不甚合理；  
# 若是实际图像中没有清晰面部特征，是否可以对图像进行背景剥离，或者图像增强等方式，通过后期手段使得面部特征明显后再进行识别。
# 
# 
...
# 回答：human_files_short中图像检测到狗狗的百分比为0%；
#   
# - `dog_files_short`中图像检测到狗狗的百分比？  
# 回答：dog_files_short中图像检测到狗狗的百分比为100%。 
...
for face in dog_files_short:
    if dog_detector(face):
        dog_count+=1
print('the rate of detect the face from human_files is {:.0%} '.format(human_count/len(human_files_short)))
print('the rate of detect the face from dog_files is {:.0%} '.format(dog_count/len(dog_files_short)))
...
# 
# __回答:__   
# - 构建卷积网络，首先构建2D卷积层，在如下模型中，搭建三层卷积层，激活函数的“relu”，并且padding方式选择“valid”，即当实际图像边界不满足卷积窗口时填充0，在第一个卷积层按照处理后的图像大小确定实际的input shape；  
# - 其次，在每个卷积层后增加最大池化层，卷积层中的fliter实际增加模型的参数，增加了维度，易产生过拟合，池化层可以降低维度；  
# - 再则，增加压平层（flatten（），）压平层属于核心层，它主要是把多维的输入一维化，常用在从卷积层到全连接层的过渡。Flatten不影响batch的大小； 
# - Dropout将在训练过程中每次更新参数时随机断开一定百分比（p）的输入神经元连接，Dropout层用于防止过拟合；  
# - 最后构建全链接层Dense，针对狗狗的种类（133种），本次的卷积模型的输出是一个133元的多元输出。
# 
...
# __回答:__   
# 
# 如下对比了VGG19和Resnet50模型的准确率，Resnet50的准确率明显更高。  
# 
# VGG网络架构weight数量相当大，很消耗磁盘空间，训练非常慢，并且在处理图像分类识别的时候，由于其全连接节点的数量较多，再加上网络比较深，比较容易产生深层网络的收敛问题；
# 
# ResNet（残差网络）与传统的顺序网络架构（如AlexNet、OverFeat和VGG）不同，其加入了y=x层(恒等映射层),可以让网络在深度增加情况下却不退化。即使是RESNET比VGG16和VGG19更深，模型的大小实际上是相当小的，特别是在使用global average pooling（全局平均水平池）代替全连接层的时候，能有效的降低模型的大小。
# 
# 
...
datagen.fit(train_VGG19)


#  randomly shift images horizontally(10% of the total width),randomly shift images vertically(10% of the total height),randomly flip images horizontally
...
batch_size=32
epochs=20
#checkpointer = ModelCheckpoint(filepath='saved_models/aug_models.weights.best.hdf5', 
#                              verbose=1, save_best_only=True)
VGG19_model.fit_generator(datagen.flow(train_VGG19, train_targets,batch_size=batch_size), 
          steps_per_epoch=train_VGG19.shape[0]//batch_size,epochs=epochs,verbose=2,callbacks=[checkpointer],validation_data=(valid_VGG19, valid_targets))
...
#checkpointer = ModelCheckpoint(filepath='saved_models/aug_models.weights.best.hdf5', 
#                              verbose=1, save_best_only=True)
Resnet50_model.fit_generator(datagen.flow(train_Resnet50, train_targets,batch_size=batch_size), 
          steps_per_epoch=train_Resnet50.shape[0]//batch_size,epochs=epochs,verbose=2,callbacks=[checkpointer],validation_data=(valid_Resnet50, valid_targets))
...
# 比较VGG19和Resnet50的准确率，明显Resnet50的准确率更高，达到了83.4%，所以选择Resnet50的模型。
...
def find_dog_breed(img_path):
    if dog_detector(img_path)==1:
        print('this picture is one kind of dogs called: {}'.format(Resnet50_predict_breed(img_path)))
    elif face_detector(img_path)==1:
        print('this picture belongs to human and look like: {}'.format(Resnet50_predict_breed(img_path)))
    else:
        print("Wrong! can't match this picture")
...
#   
# 回答：  
# 1. 输出的结果比我预想的要好，没有把马和猫识别成狗，也正确的区分了狗和人，以及给出了狗的品种；  
# 2. 改进模型的想法：a.更改dropout的参数，将过拟合最小化； b. 更改输出层的激活函数，避免梯度消失；c. 选择合适的图片增强参数，增强图片的识别。
...
# ![IMG_4.jpg](attachment:IMG_4.jpg)

# - IMG_5

# ![IMG_5.jpg](attachment:IMG_5.jpg)

# - IMG_6

# ![IMG_6.jpg](attachment:IMG_6.jpg)
...
find_dog_breed('/Users/juni/Downloads/IMG_1.jpg')


# In[51]:


find_dog_breed('/Users/juni/Downloads/IMG_2.jpg')


# In[53]:


find_dog_breed('/Users/juni/Downloads/IMG_3.jpg')


# In[54]:


find_dog_breed('/Users/juni/Downloads/IMG_4.jpg')


# In[55]:


find_dog_breed('/Users/juni/Downloads/IMG_5.jpg')


# In[56]:


find_dog_breed('/Users/juni/Downloads/IMG_6.jpg')
